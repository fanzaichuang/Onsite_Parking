...
def calc_all_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=STEP_SIZE):
    ...
    # 对于所有路径生成局部路径的x、y、yaw和directions并转换坐标系
    for path in paths:
        x, y, yaw, directions = \
            generate_local_course(path.L, path.lengths,
                                  path.ctypes, maxc, step_size * maxc)

        # 转换坐标系，将局部路径的坐标点 (x, y) 转换到全局坐标系下
        # 对于每个坐标点 (ix, iy)，应用了一个旋转矩阵，将其转换到全局坐标系，并加上了起始点 q0 的坐标偏移
        # math.cos(-q0[2]) 和 math.sin(-q0[2]) 分别是起始点的方向的余弦值和正弦值
...
def set_path(paths, lengths, ctypes):
    ...
    # 检测paths中的路径是否与生成的path相同，相同的话直接返回paths
    for path_e in paths:
    ...
    # 如果长度大于其最大要求的长度，则直接返回paths
    if path.L >= MAX_LENGTH:
        return paths

    # 断言path.L一定大于0.01，否则会报错
    assert path.L >= 0.01
    # 将生成的path添加到paths中
    paths.append(path)
...
def generate_path(q0, q1, maxc):
    # 计算从起始点到目标点的相对位移dx、dy和角度变化 dth
    dx = q1[0] - q0[0]
    dy = q1[1] - q0[1]
    dth = q1[2] - q0[2]
    # 计算了在起始点方向上的位移x和y，这里应用了旋转矩阵的变换
    c = math.cos(q0[2])
    s = math.sin(q0[2])
    x = (c * dx + s * dy) * maxc
    y = (-s * dx + c * dy) * maxc

    # 创建一个空列表paths用于存储生成的路径
    paths = []
    # 分别调用了六个函数 SCS()、CSC()、CCC()、CCCC()、CCSC() 和 CCSCC()
    # 在这些路径的基础上生成更多的路径，并将它们添加到 paths 中
